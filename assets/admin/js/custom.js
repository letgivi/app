...
function showModal(contentHtml, footerHtml)
{
    ...
    // Catch close event
    $('#genericModalContainer').on('hidden.bs.modal', function (e) {
        // Revert any z-index overrides, this allows for modals on top of modals
        $('.modal').css('z-index', '');
    });
...
{
    $('.dataTables_wrapper select, .dataTables_wrapper input[type=text], .dataTables_wrapper input[type=number]').addClass("form-control");
    $('.dataTables_wrapper select, .dataTables_wrapper input[type=text], .dataTables_wrapper input[type=number]').addClass("input-sm");
    reloadCheckedItems();
    setupFlatCheckboxes();
    setTableLoaded();
    setupTooltips();
