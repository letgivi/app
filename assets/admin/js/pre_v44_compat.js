...
$(document).ready(function () {
    $('.main_container > div.row .col_12 .widget_inside .col_12').not('.ignore-compat').wrap('<div class=\'x_panel\'></div>');
    $('.main_container > div.row .col_12 form').addClass('form-horizontal form-label-left');
    $('.main_container > div.row .col_12 input[type=text]:not(.button), .main_container > div.row .col_12 input[type=number]:not(.button), .main_container > div.row .col_12 input[type=password]:not(.button), .main_container > div.row .col_12 select, .main_container > div.row .col_12 textarea').addClass('form-control');
    $('.main_container > div.row .col_12 form .col_8 .form > div.clearfix').addClass('form-group');
    $('.main_container > div.row .col_12 form label').not('.ignore-compat').addClass('control-label col-md-3 col-sm-3 col-xs-12');
    ...
    $('.main_container > div.row .col_12 form .x_title p').each(function() {
        $(this).parent().next('.x_content').prepend($(this));
    });
