...
$(document).ready(function () {
    ...
    $(window).keyup(function (e) {
        // don't do anything in textareas
        target = $(e.target);
...
function showFile(fileId, passChecks)
{
    if(typeof(passChecks) === 'undefined') {
        passChecks = false;
    }
    ...
    loadAjaxContent('#main-ajax-container', ACCOUNT_WEB_ROOT + "/ajax/file_details", {u: fileId, p: passChecks}, successCallback);
}
...
function updatePerPage(key, label, ele, additionalOptions)
{
    ...
    var loadImagesFunc = 'loadImages';
    if(typeof(additionalOptions.loadImagesFunc) !== 'undefined') {
        loadImagesFunc = additionalOptions.loadImagesFunc;
    }

    // Ensure the function exists before calling
    if (typeof window[loadImagesFunc] === 'function') {
        window[loadImagesFunc](currentPageType, null, 1, perPage, '', additionalOptions);
    } else {
        console.error(`Function ${loadImagesFunc} is not defined.`);
    }
}
...
function updateSorting(key, label, ele, additionalOptions)
{
    ...
    // Ensure the function exists before calling
    if (typeof window[loadImagesFunc] === 'function') {
        window[loadImagesFunc](currentPageType, null, 1, 0, filterOrderBy, additionalOptions);
    } else {
        console.error(`Function ${loadImagesFunc} is not defined.`);
    }
}
...
function loadUploader(showUploader) {
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            // Reload the table for background URLs
            if(typeof(setupBackgroundUrlDownloads) !== 'undefined') {
                setupBackgroundUrlDownloads();
            }
...
function assignLiOnClick()
{
    ...
    $(".fileManager .fileIconLi.owned-image a.fileDownload").click(function (e) {
        ...
        return showFileMenu(liElement, e, true);
    });
    $(".fileManager .folderIconLi.owned-folder a.fileDownload").click(function (e) {
        liElement = $(this).parents('.folderIconLi');
        return showFolderMenu(liElement, e, true);
    });
    $(".fileManager .fileIconLi .itemCheckbox").click(function (e) {
        e.stopPropagation();
        liElement = $(this).parents('.fileIconLi');

        // select file
        fileId = $(liElement).attr('fileId');
        toggleSelectedFile(fileId);
    });
    $(".fileManager .folderIconLi .itemCheckbox").click(function (e) {
        e.stopPropagation();
        liElement = $(this).parents('.folderIconLi');

        // select folder
        folderId = $(liElement).attr('folderId');
        toggleSelectedFolder(folderId);
    });
...
function unbindLiOnClick()
{
    ...
    $(".fileManager .fileIconLi .itemCheckbox").unbind('click');
    $(".fileManager .folderIconLi .itemCheckbox").unbind('click');
    unbindLiRightClick();
}
...
function showFileHistory(fileId)
{
    ...
    $.ajax({
        type: "GET",
        url: ACCOUNT_WEB_ROOT + "/ajax/file_history/"+fileId,
        dataType: 'json',
            ...
            jQuery('#addEditFolderModal .modal-content').html(json.html);
                $('#historyTable').dataTable({
                    ...
                    "fnDrawCallback": function (oSettings) {
                        hideLoaderModal();
                        jQuery('#addEditFolderModal').modal('show', {backdrop: 'static'}).on('shown.bs.modal');
                    },
                    "oLanguage": {
                        "oPaginate": {
                        ...
                        "sEmptyTable": "There is no history for this file."
                    }
                });
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            hideLoaderModal();
            showErrorNotification(textStatus);
        }
    });
}
...
function copyItems(fileId, folderId) {
    showLoaderModal();

    // allow for passed params
    fileIds = '';
    folderIds = '';
    if(typeof(fileId) !== "undefined" || typeof(folderId) !== "undefined") {
    ...
    jQuery('#addEditFolderModal .modal-content').load(ACCOUNT_WEB_ROOT + "/ajax/copy_to_folder", {fileIds: fileIds, folderIds: folderIds, currentFolderId: currentAlbumId }, function () {
        hideLoaderModal();
        jQuery('#addEditFolderModal').modal('show', {backdrop: 'static'}).on('shown.bs.modal', function () {
            $('#addEditFolderModal input').first().focus();
        });
...
function moveFilesSelectFolder(fileId, folderId) {
    showLoaderModal();
    ...
    jQuery('#addEditFolderModal .modal-content').load(ACCOUNT_WEB_ROOT + "/ajax/move_to_folder", {fileIds: fileIds, folderIds: folderIds, currentFolderId: currentAlbumId }, function () {
        hideLoaderModal();
        jQuery('#addEditFolderModal').modal('show', {backdrop: 'static'}).on('shown.bs.modal', function () {
            $('#addEditFolderModal input').first().focus();
        });
...
function processMoveFilesSelectedFolder() {
    return moveFilesIntoFolder($('#moveFolderId').val());
}
...
function hideOpenContextMenus()
{
    ...
    if (typeof $.vakata !== 'undefined' && $.vakata.context && typeof $.vakata.context.hide === 'function') {
        $.vakata.context.hide();
    }
